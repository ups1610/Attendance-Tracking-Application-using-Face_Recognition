...
mydata=[]
class Attendance_Database_System:
    def __init__(self,root):
        ...
        #background image
        ''' img=Image.open(r"E:\python\MS_project\Face_Recognition_System\image_reource\Frame 5.png")
        img=img.resize((1530,800),Image.ANTIALIAS)
        self.bg_img=ImageTk.PhotoImage(img)

        img_label=Label(self.root,image=self.bg_img)
        img_label.place(x=0,y=0,width=1520,height=800)'''

        #---banner
        frame = Frame(self.root,bd=2,relief=RIDGE,bg='black',width=1530,height=200)
        frame.pack()
    

        bg_img=Image.open(r"E:\python\MS_project\Face_Recognition_System\image_reource\att_db.png")
        bg_img=bg_img.resize((1530,225),Image.ANTIALIAS)
        ...
        #--homepage icon
        h_icon=Image.open(r"E:\python\MS_project\Face_Recognition_System\image_reource\icon.png")
        h_icon=h_icon.resize((40,40),Image.ANTIALIAS)
        self.img_icon=ImageTk.PhotoImage(h_icon)
                
        b_icon=Button(bg_img_label,image=self.img_icon,cursor='arrow',activebackground='grey',activeforeground='orange',border=0,background='white')
        b_icon.place(x=6,y=13,height=40,width=40)
        ...
        #--------------Main frame---------------------------------
        main_frame=Frame(self.root,bd=2,bg='black',border=0,width=1510,height=540)
        main_frame.pack(pady=10,expand=True)

        #--------------------------------------left frame-----------------------------------

        l_frame=LabelFrame(main_frame,bd=2,bg='white',relief=RIDGE,text="Student Information",font='SUNKIN',fg='dark red')
        l_frame.place(x=10,y=5,width=720,height=525)

        img_lft=Image.open(r"E:\python\MS_project\Face_Recognition_System\image_reource\8732.jpg")
        img_lft=img_lft.resize((470,175),Image.ADAPTIVE)
        self.img_lft=ImageTk.PhotoImage(img_lft)

        img_lft_label=Label(l_frame,image=self.img_lft,activebackground='white',activeforeground='white',background='white')
        img_lft_label.place(x=130,y=0,width=470,height=175)
        
        head_txt=Label(main_frame,text="Attendance Information",font=(SUNKEN,'15','bold'),bg='white',fg='black')
        head_txt.place(x=250,y=190)

        inner_left_frame=Frame(main_frame,bd=2,bg='lavender',relief=RIDGE)
        inner_left_frame.place(x=20,y=215,width=700,height=305)

        #------------left Label Entry------------
        #--Attendance Id:
        atd_label=Label(inner_left_frame,text='Attendance ID:',font=(SUNKEN,13,'bold'),bg='lavender') 
        atd_label.grid(row=0,column=0,padx=10,sticky=W)

        atd_entry=ttk.Entry(inner_left_frame,width=20,font=('SUNKEN',13,'normal'),background='black',textvariable=self.var_attendance_id)
        atd_entry.grid(row=0,column=1,padx=10,pady=5,sticky=W)

        #--Name:
        name_label=Label(inner_left_frame,text='Name:',font=(SUNKEN,13,'bold'),bg='lavender') 
        name_label.grid(row=0,column=2,padx=10,sticky=W)

        name_entry=ttk.Entry(inner_left_frame,width=20,font=('SUNKEN',13,'normal'),background='black',textvariable=self.var_attendance_name)
        name_entry.grid(row=0,column=3,padx=10,pady=5,sticky=W)

        #--Roll:
        roll_label=Label(inner_left_frame,text='Roll No:',font=(SUNKEN,13,'bold'),bg='lavender') 
        roll_label.grid(row=1,column=0,padx=10,sticky=W)

        roll_entry=ttk.Entry(inner_left_frame,width=20,font=('SUNKEN',13,'normal'),background='black',textvariable=self.var_attendance_roll)
        roll_entry.grid(row=1,column=1,padx=10,pady=5,sticky=W)

        #--department:
        dep_label=Label(inner_left_frame,text='Department:',font=(SUNKEN,13,'bold'),bg='lavender') 
        dep_label.grid(row=1,column=2,padx=10,sticky=W)

        roll_entry=ttk.Entry(inner_left_frame,width=20,font=('SUNKEN',13,'normal'),background='black',textvariable=self.var_attendance_dep)
        roll_entry.grid(row=1,column=3,padx=10,pady=5,sticky=W)

        #--Date:
        date_label=Label(inner_left_frame,text='Time:',font=(SUNKEN,13,'bold'),bg='lavender') 
        date_label.grid(row=2,column=0,padx=10,sticky=W)

        date_entry=ttk.Entry(inner_left_frame,width=20,font=('SUNKEN',13,'normal'),background='black',textvariable=self.var_attendance_date)
        date_entry.grid(row=2,column=1,padx=10,pady=5,sticky=W)
 

        #--Time:
        time_label=Label(inner_left_frame,text='Time:',font=(SUNKEN,13,'bold'),bg='lavender') 
        time_label.grid(row=2,column=2,padx=10,sticky=W)

        time_entry=ttk.Entry(inner_left_frame,width=20,font=('SUNKEN',13,'normal'),background='black',textvariable=self.var_attendance_time)
        time_entry.grid(row=2,column=3,padx=10,pady=5,sticky=W)
        
        #----Attendance status
        status_label=Label(inner_left_frame,text='Status:',font=(SUNKEN,13,'bold'),bg='lavender') 
        status_label.grid(row=3,column=0,padx=10,sticky=W)

        status_combo=ttk.Combobox(inner_left_frame,font=("SUNKEN",13,'normal'),width=18,state='readonly',textvariable=self.var_attendance_status)
        ...
        btn_frame.place(x=3,y=200,width=690,height=37)
        
        save_btn=Button(btn_frame,text='CSVin',width=16,font=(SUNKEN,13,'bold'),bg='green',fg='white',command=self.import_csv)
        save_btn.grid(row=0,column=0)

        del_btn=Button(btn_frame,text='CSVout',width=16,font=(SUNKEN,13,'bold'),bg='green',fg='white',command=self.export_csv)
        del_btn.grid(row=0,column=1)

        update_btn=Button(btn_frame,text='UPDATE',width=17,font=(SUNKEN,13,'bold'),bg='green',fg='white')
        update_btn.grid(row=0,column=2)

        reset_btn=Button(btn_frame,text='RESET',width=17,font=(SUNKEN,13,'bold'),bg='green',fg='white',command=self.reset)
        reset_btn.grid(row=0,column=3)
        
        #---------right-frame------------------
        r_frame=LabelFrame(main_frame,bd=2,bg='white',relief=RIDGE,text='Student Details',font='SUNKEN',fg='dark red')
        r_frame.place(x=740,y=5,width=760,height=525)

        img_rht=Image.open(r"E:\python\MS_project\Face_Recognition_System\image_reource\attt.jpg")
        img_rht=img_rht.resize((500,185),Image.ADAPTIVE)
        self.img_rht=ImageTk.PhotoImage(img_rht)

        img_rht_label=Label(r_frame,image=self.img_rht,activebackground='white',activeforeground='white',background='white')
        img_rht_label.place(x=120,y=0,width=500,height=185)

        #------table frame-----------------
        data_frame=Frame(r_frame,bd=2,bg='white',relief=RIDGE)
        data_frame.place(x=8,y=180,width=740,height=310)
...
if __name__ == "__main__":
    ...
    object=Attendance_Database_System(root)
    root.mainloop()           